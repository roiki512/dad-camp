...
window.DADCAMP_PROFILE = {
  ...
  constraints: {
    ...
    readingAfterLibrary: "NO reading-of-books activity ('learn-readaloud' Read-Aloud OR 'quiet-soloreading' Cozy Solo Reading) may be scheduled BEFORE a library visit — not on an earlier day, and not earlier the same day as the trip. Put reading activities on the library day AFTER the trip, or the 1–2 days after (fresh books). On the library day itself, make the morning Learning Block a NON-reading subject (the library trip is that day's literacy anchor; e.g. swap reading↔another subject in the rotation). If there's no library that week, skip solo reading; read-aloud is OK with existing books.",
